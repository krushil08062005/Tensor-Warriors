...
const PendingCases = () => {
    ...
    const getAvailableCategories = () => {
      ...
      Object.keys(mediaAnalysis).forEach((key) => {
        if (
          ...
          (typeof mediaAnalysis[key] === "object" ||
            typeof mediaAnalysis[key] === "number")
        ) {
          categories.push(key);
        }
      });
                ...
                !["prob", "safe", "raw", "partial"].includes(category) && (
                  <div className="mt-2 space-y-1">
                    {Object.entries(mediaAnalysis[category])
                      .filter(
                        ([subKey]) =>
                          !["prob", "safe", "raw", "partial"].includes(subKey)
                      )
                      .map(([subKey, subValue]) => (
                        <div
                          key={subKey}
        ...
        {mediaAnalysis.is_detected && (
          <div className="mt-3 flex items-start gap-2 p-3 bg-red-50 border border-red-100 rounded-lg">
            ...
            <p className="text-sm text-red-700">
              ...
              {mediaAnalysis.flagged_categories?.join(", ") ||
  ...
  const handleUpdateSeverity = async (caseId, newSeverity) => {
    try {
      ...
      setPendingCases((prev) =>
        prev.map((c) => (c.id === caseId ? { ...c, severity: newSeverity } : c))
      );
  ...
  const handleMarkAsFake = async (caseId) => {
    if (
      !window.confirm(
        "Are you sure you want to mark this case as fake? This will permanently delete it."
      )
    ) {
      return;
    }
  ...
  const getCaseTimeline = (caseItem) => {
    return [
      {
        time: new Date(caseItem.reported_at).toLocaleTimeString([], {
          hour: "2-digit",
          minute: "2-digit",
        }),
      ...
      {
        time: new Date(
          new Date(caseItem.reported_at).getTime() + 30 * 60000
      ...
      },
      {
        ...
        description:
          "Awaiting moderator approval to proceed with investigation.",
      },
    ];
  };
          ...
          >
            <div className="p-6 space-y-6">
              ...
              {selectedCase.media_analysis && (
                <div className="mt-6">
                  <AnalysisCategories
                    mediaAnalysis={selectedCase.media_analysis}
                  />
                </div>
              )}
            </div>
